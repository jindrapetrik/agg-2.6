...
/**
 * Path converter that shortens paths by a specified amount from both ends.
 * Translates conv_shorten_path from C++ AGG library.
 */
public class ConvShortenPath implements VertexSource {
    private VertexSource source;
    private VcgenVertexSequence generator;
    private int status;
    ...
    public ConvShortenPath(VertexSource source) {
        this.source = source;
        this.generator = new VcgenVertexSequence();
    ...
    public void setShorten(double s) {
        generator.setShorten(s);
    }

    public double getShorten() {
        return generator.getShorten();
    }
